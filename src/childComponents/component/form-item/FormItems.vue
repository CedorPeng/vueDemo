    ...
    <Form :ref="refForm" :model="formVal" :rules="rules" :label-position="labelPosition">
        <Row :gutter="gutter" v-for="(formCols,rIndex) in formRows" :key="rIndex">
            <Col :span="24/formCols.length" v-for="(component,cIndex) in formCols"  :key="cIndex">
                <component :is="component.name"  v-bind="component.attr" v-model="formVal[component.attr.prop]" v-if="['form-item-input','form-item-select'].includes(component.name)"></component>
                <component :is="component.name"  v-bind="component.attr" :startDate.sync="formVal[component.attr.startDateSync]" :endDate.sync="formVal[component.attr.endDateSync]" v-else-if="component.name==='form-item-datepicker'"></component>
